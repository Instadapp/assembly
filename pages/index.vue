<template>
  <div>
    ...
    <div class="mt-10 grid w-full grid-cols-1 gap-4 sm:grid-cols-3 xl:gap-6">
      <nuxt-link v-for="app in apps" :key="app.id" :to="app.url"
        ...
        <div style="background: radial-gradient(42.15% 42.15% at 48.94% 48.94%, #D6DAE0 75.67%, #F0F3F9 100%), #C4C4C4;"
          ...
          <div class="w-16 h-16 rounded-full flex items-center justify-center bg-[#1874FF]">
            <component :is="app.icon" class="w-10 h-10 text-white" />
          </div>
        </div>

        <h2 class="mt-4 font-semibold text-19">{{ app.name }}</h2>
        <p class="mt-2 text-14 font-regular text-grey-dark opacity-90" v-html="app.description">
        </p>
      </nuxt-link>
    </div>
  </div>
