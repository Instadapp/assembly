...
import benqiABI from "~/abis/read/benqi.json";
...
export function useBenqiPosition(
  ...
  const fetchPosition = async () => {
    ...
    const benqiRawData = await resolverInstance.methods
      .getPosition(activeAccount.value.address, tokensArr)
      .call();

    const newPos = calculateBenqiPosition(
      benqiRawData,
  ...
  onEvent("protocol::benqi::refresh", refreshPosition);
...
function calculateBenqiPosition(
  res: any[],
